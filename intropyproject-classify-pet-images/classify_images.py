...
def classify_images(images_dir, results_dic, model):
    ...
    for filename, labels in results_dic.items():
        ...
        # Run classifier function to classify the image and get the class label
        class_label = classifier(img_path, model).lower().strip()

        # Add the classifier label and the comparison of the labels to the results dictionary
        label = labels[0]
        label_match = int(label in class_label)
        results_dic[filename].extend([class_label, label_match])
